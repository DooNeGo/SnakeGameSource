...
internal sealed class CollisionNotifier : Component
{
    ...
    {
        if (component is not CollisionNotifier notifier) return false;
        CollisionEnter = (Action<GameObject>?)notifier.CollisionEnter?.Clone();
        return true;
    }

    protected override void OnCollisionEnter(GameObject gameObject) => CollisionEnter?.Invoke(gameObject);
}